	...
	render() {
		return (
			<div className="VoteViewLoginInfo">
				...
				{this.props.data.isSuperUser ? <button className="btn btn-md btn-primary btn-transparent" onClick={() => SocketManager.closeSession()}>Close session</button> : null} <button className="btn btn-md btn-primary btn-transparent" onClick={() => SocketManager.leaveSession()}>Leave session</button><br></br><br></br>
			</div>
		);
	}
}
