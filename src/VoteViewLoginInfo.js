	...
	render() {
		return (
			<div className="VoteViewLoginInfo">
				SessionID: <b>{this.props.data.sessionId}</b>{this.props.data.isSuperUser ? <button onClick={this.refresh.bind(this)}>Close session</button> : null} <button onClick={() => window.location.reload()}>Close without emits (for testing only) </button><br></br>
